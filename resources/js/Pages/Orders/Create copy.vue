<script setup>
    ...
    const totalPrice = computed(()=>{
        ...
        itemList.value.forEach(item=>{
            total += item.pcs*item.sales_price;
        })
        return total
    })
                        ...
                        <form @submit.prevent="storeOrder" >
                            ...
                            <div class="container px-5 py-8 mx-auto">
                                ...
                                <div class="flex flex-wrap -m-2">
                                    ...
                                    <div class="p-2 w-full">
                                    <div class="relative">
                                        <label for="order_info" class="leading-7 text-sm text-gray-600">詳細</label>
                                ...
                                <table class="bg-white table-auto w-full text-center whitespace-no-wrap">
                                    ...
                                    <tbody>
                                        <tr v-for="(item, index) in itemList" :key="index">
                                            <td >{{ index + 1 }}</td>
                                            <td>
                                                <select v-model="item.id" @change="onItemChange(index)" class="w-60">
                                            ...
                                            <td class="w-40">
                                                <input type="number" v-model.number="item.sales_price" class="w-24 text-right" />
                                            </td>
                                            <td class="w-32">
                                                <select v-model="item.pcs" class="w-16">
                                                    <option v-for="q in pcs" :key="q" :value="q">{{ q }}</option>
                                                </select>
                                            </td>

                                            <td class="w-60">{{ item.pcs * item.sales_price }}</td>
                                        </tr>
